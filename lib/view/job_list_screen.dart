...
class JobListScreen extends StatefulWidget {
  ...
  const JobListScreen({super.key, required this.searchQuery, this.resumeSkills});
...
class _JobListScreenState extends State<JobListScreen> {
  ...
  void initState() {
    ...
    _matchEngine = MatchEngine(swipeItems: []); // <-- Fix here
    fetchResumeAndJobs();
  }

  Future<void> fetchResumeAndJobs() async {
    final skills = await fetchLatestResumeSkills();
    setState(() {
      _resumeSkillsFromFirestore = skills ?? '';
    });
    fetchJobs();
  }
  ...
  Widget buildJobCard(dynamic job) {
    ...
    // Use resume skills fetched from Firestore
    double matchScore = 0.0;
    List<String> resumeSkillsList = [];
    List<String> jobSkills = [];
    if (_resumeSkillsFromFirestore.isNotEmpty) {
      resumeSkillsList = _resumeSkillsFromFirestore.split(',').map((e) => e.trim()).toList();
      jobSkills = description.split(RegExp(r'\W+')).where((e) => e.isNotEmpty).toList();
      matchScore = cosineSimilarity(resumeSkillsList, jobSkills) * 100;
    }

    print('Resume: $resumeSkillsList, Job: $jobSkills, Score: $matchScore');
    print('Resume skills from Firestore: $_resumeSkillsFromFirestore');
            ...
            child: Column(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Center(
                    child: Text(
                      _resumeSkillsFromFirestore.isEmpty
                        ? "No resume skills found"
                        : "Matching Score: ${matchScore.toStringAsFixed(1)}%",
                      style: TextStyle(
  ...
  Future<String?> fetchLatestResumeSkills() async {
    ...
    final resumes = userDoc.data()?['resumes'] as List<dynamic>?;

    if (resumes == null || resumes.isEmpty) return null;

    // Get the latest resume (assuming last is latest)
    final latestResume = resumes.last as Map<String, dynamic>;
    final skills = latestResume['skills'];
    if (skills is String) return skills;
    if (skills is List) return skills.join(',');
    return null;
  }

  String _resumeSkillsFromFirestore = '';
}

// Utility function for cosine similarity
double cosineSimilarity(List<String> a, List<String> b) {
  final setA = Set<String>.from(a.map((e) => e.toLowerCase()));
  final setB = Set<String>.from(b.map((e) => e.toLowerCase()));
  final intersection = setA.intersection(setB).length;
  if (setA.isEmpty || setB.isEmpty) return 0.0;
  return intersection / sqrt(setA.length * setB.length);
}

// SavedJobsScreen class (keep your implementation here)
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(job['employer_name'] ?? ''),
